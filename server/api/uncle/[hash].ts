...
export default defineEventHandler((event) => {
  return new Promise<api_pb.GetUncleByHashResponse.AsObject>((resolve, reject) => {
    const hash = event.context.params?.hash ?? ''
    const request = new api_pb.GetBlockByBlockHashRequest()
    request.setHash(hash)

    const { client } = useClient()
    client.getUncleByHash(request, (err: any, response: any) => {
      if (err) reject(err)
